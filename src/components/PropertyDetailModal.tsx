  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6 animate-fadeIn">
      ...
      <div className="relative bg-white w-full max-w-4xl rounded-3xl shadow-2xl border border-slate-200 overflow-hidden my-auto max-h-[92vh] flex flex-col">
        ...
        {/* Sticky Header */}
        <div className="p-4 sm:px-6 bg-[#102033] text-white flex items-center justify-between border-b border-slate-800">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`p-2 rounded-xl border transition-colors cursor-pointer ${
                isSaved ? 'bg-[#167A5A] border-emerald-500 text-white' : 'bg-slate-800 border-slate-700 text-slate-300 hover:text-white'
              }`}
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-6">
          ...
          <div className="space-y-3">
            <div className="relative aspect-[16/9] sm:aspect-[21/9] rounded-2xl overflow-hidden bg-slate-100 border border-slate-200">
              ...
              />
              <div className="absolute top-3 left-3 bg-[#102033]/90 text-white text-xs font-bold px-3 py-1 rounded-full border border-white/20 flex items-center gap-1.5">
                ...
                {property.images.map((img, idx) => (
                  <button
                    ...
                    className={`relative w-20 h-14 rounded-xl overflow-hidden border-2 flex-shrink-0 cursor-pointer transition-all ${
                      activeImageIndex === idx ? 'border-[#167A5A] scale-105' : 'border-transparent opacity-70 hover:opacity-100'
                    }`}
                  >
          ...
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <div className="text-2xl font-extrabold text-[#102033] mt-1">
                {priceFormatted}
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex-1 sm:flex-initial px-5 py-3 bg-[#167A5A] hover:bg-[#126248] text-white font-bold text-xs rounded-xl flex items-center justify-center gap-2 transition-all shadow-xs"
              >
          ...
          <div className="p-4 rounded-2xl bg-emerald-50/80 border border-emerald-200 space-y-2">
            <div className="flex items-center gap-2 text-[#167A5A] font-extrabold text-xs">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-5">
              <div>
                <h4 className="text-xs font-extrabold text-[#102033] uppercase tracking-wider mb-2">Property Overview</h4>
              ...
              {property.features && property.features.length > 0 && (
                <div>
                  <h4 className="text-xs font-extrabold text-[#102033] uppercase tracking-wider mb-3">Key Features & Highlights</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {property.features.map((feat, idx) => (
                      <div key={idx} className="flex items-center gap-2 text-xs font-semibold text-slate-700 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                        <CheckCircle2 className="w-4 h-4 text-[#167A5A] flex-shrink-0" />
                        <span>{feat}</span>
                      </div>
                    ))}
                  </div>
              ...
              {property.nearbyLandmarks && property.nearbyLandmarks.length > 0 && (
                <div>
                  <h4 className="text-xs font-extrabold text-[#102033] uppercase tracking-wider mb-2">Neighborhood Proximity</h4>
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
                ...
                <div className="space-y-2 text-xs text-slate-700 font-medium">
                  ...
                  <div className="flex justify-between py-1 border-b border-slate-200/60">
                    <span className="text-slate-500">Title Document</span>
                    <span className="font-bold text-[#167A5A]">{property.titleStatus}</span>
                  </div>
                  {property.sizeSqm && (
                    <div className="flex justify-between py-1 border-b border-slate-200/60">
                      <span className="text-slate-500">Land Size</span>
                      <span className="font-bold">{property.sizeSqm} sqm</span>
                    </div>
                  )}
                  {property.bedrooms && (
                    <div className="flex justify-between py-1 border-b border-slate-200/60">
                      <span className="text-slate-500">Bedrooms</span>
                      <span className="font-bold">{property.bedrooms} Ensuite</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              {property.paymentPlan?.available && (
                <div className="p-4 rounded-2xl bg-white border border-slate-200 shadow-xs space-y-3">
                  <div className="flex items-center gap-1.5 text-[#102033] font-bold text-xs">
                    <Calculator className="w-4 h-4 text-[#167A5A]" />
                  ...
                  <div className="space-y-2 text-xs">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full accent-[#167A5A]"
                      />
                    </div>
                        ...
                        {[3, 6, 12].map((m) => (
                          <button
                            ...
                            className={`py-1 rounded-md text-[11px] font-bold border transition-colors cursor-pointer ${
                              tenorMonths === m
                                ? 'bg-[#102033] text-white border-[#102033]'
                    ...
                    <div className="pt-2 border-t border-slate-100 flex justify-between items-center">
                      ...
                      <span className="text-sm font-extrabold text-[#102033]">
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-500 font-medium">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-[#102033] text-white flex items-center justify-center font-bold text-xs">
              LP
            </div>
            <div>
              <div className="font-bold text-[#102033]">{property.developerInfo.name}</div>
          ...
          <div className="flex items-center gap-2">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-5 py-2 bg-[#102033] hover:bg-[#167A5A] text-white rounded-xl font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
            >
